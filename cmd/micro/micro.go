...
func parseCurlyArg(args []string) int {
	...
	for _, a := range args {
		...
		if len(match) == 3 && match[2] != "" {
		    ...
		    searchflagr := regexp.MustCompile(`^\+/(.*)?$`)
	     	match2 := searchflagr.FindStringSubmatch(a)
		    if len(match2)>0 {
		                   if match2[0]!="" {
		                           //    screen.TermMessage(match2[0])
								searchFlag=match2[0][2:len(match2[0])]
		                     }
		                   if match2[0]=="" {
		                             _="" //   screen.TermMessage(nil)
		                     }
		          } else {
		         //  if a!="" {
...
func main() {
	...
	args := flag.Args()
	startupInt := parseCurlyArg(args) // your regex helper
//	config.StartupInt = startupInt
...
//	bf := b[0]
	if len(b) == 0 {
	  ...
	  if (buffer.IsMatch1 != true){
		var strs []string
		obuffs := b
		for _, obj := range obuffs {
			//if obj.GetName() {
			   strs = append(strs, obj.GetName());
			//}
		}
		var patidx int
		var isMatch2 bool
	    for idx, strthing := range strs {
	     	pattern := `\{\d*\}` 
		    str1 := strthing
	     	isMatch2, _ = regexp.MatchString(pattern, str1)
	     	if isMatch2 {
	     	   patidx = idx
	     	   break;
	     	   //_ := patidx
	   ...
	   if isMatch2 {
	 //  screen.TermMessage("theres an empty buf",
	  // 		strs[patidx])
		   obuffs[patidx].Close()
		   var indexToRemove=patidx
		   if len(b)==1 {
				b[indexToRemove].SetName("a")
				 	filename:=""
		   	var input []byte
		   // Create a new slice by appending the parts before and after the removed index
		   			        b = append(b[:indexToRemove], b[indexToRemove+1:]...)

	        	b = append(b, buffer.NewBufferFromString(string(input), filename, buffer.BTDefault))
    ...
    var str2 = searchFlag
	action.InitTabs(b)
	var active = action.MainTab()

    
	var pane = active.CurPane()
//	 var cursorc = pane.Buf.GetActiveCursor()
//	 cursorc=pane.Cursor
    pane.Search(str2, false, true)  
  //  cursorc.AddLineToSelection()
  if (startupInt-1)>=1 {
    pane.Search(str2, false, true) 
    startupInt-=1
  }   
 ...
 // cursorc.SetSelectionStart(buffer.Loc{42,1})
 //  cursorc.SetSelectionEnd(buffer.Loc{43,7})

   //	screen.TermMessage(string(cursorc.GetSelection())) 
//	screen.TermMessage(cursorc.Loc.X)
//	screen.TermMessage(cursorc.Loc.Y)
//	screen.TermMessage(pane.Name())

//	screen.TermMessage(la.S.Match)
    ...
    var window display.TabWindow
 //   var t action.TabList
    var t = action.Tabs // localized global TabList
    
    var nms = ""
    for outidx, p := range t.List {
       for inidx, pn := range p.Panes {
            nms = nms+strconv.Itoa(outidx)+pn.Name()
            outidx=inidx
    		}
    	if len(p.Panes)>0 {
    	   
        //	p.RemovePane(p.Active);
            _=""
    	}
    	}
  //  t.CloseTerms()
