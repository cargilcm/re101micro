...
func NewBufferFromFileAtLoc(path string, btype BufType, cursorLoc Loc) (*Buffer, error) {
    ...
    var str = "did it"
	slice := []byte(str)
    var pos = Loc{0, 0} //ParseCursorLocation("0:0")
    var str2 = "Cat"
    //var ptr *string
    // ptr = &str2
...
/*	pattern := `\{\d*\}` 
	str1 := filename
	IsMatch1, _ := regexp.MatchString(pattern, str1)
    if (IsMatch1 != true){*/
					
		// Compile the regular expression
		// File does not exist -- create an empty buffer with that name
		buf = NewBufferFromString("", filename, btype)
//		util.SafeWrite("safew.txt",slice,false)
		_=slice
		var la = buf.LineArray
		buf.LastSearch=str2
		la.SearchMatch(buf,pos)	
	/*	} else {
			buf = NewBufferFromString("", "", btype)
		}*/
	} else if err != nil {
		return nil, err
	} else {
